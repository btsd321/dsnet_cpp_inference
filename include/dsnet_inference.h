...
struct SuctionScores {
    ...
    SuctionScores() 
        : seal_score(0.0f), wrench_score(0.0f), feasibility_score(0.0f), 
          object_size_score(0.0f), composite_score(0.0f) {}
};
...
public:
     ...
     * @param config 推理配置
     */
    DSNetInference(const std::string& model_path, const InferenceConfig& config = InferenceConfig());
     ...
     * @return 最佳吸取点列表
     */
    std::vector<SuctionPoint> getBestSuctionPoints(
