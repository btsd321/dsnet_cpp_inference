...
bool InferenceResult::saveToTextFile(const std::string& filename) const
{
    ...
    file << "基本信息:\n";
    file << "  推理时间: " << std::fixed << std::setprecision(3) << inference_time_ms_ << " ms\n";
    file << "  点云大小: " << getPointCloudSize() << " 个点\n";
    file << "  最佳点数量: " << best_points_.size() << " 个\n\n";
    ...
    file << "  密封评分 - 最小值: " << stats.seal_stats.min_score
         ...
         << ", 平均值: " << stats.seal_stats.mean_score << ", 标准差: " << stats.seal_stats.std_dev
         << "\n";
