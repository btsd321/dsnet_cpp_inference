#include "dsnet_// FarthestPointSampling implementation - 最远点采样算法
std::vector<int> FarthestPointSampling::sample(const Eigen::MatrixXf& points, int num_samples)
{
    ference / dsnet_pointnet_backbone.h "
...
#include <random>
    ...
    {
        // 计算两点之间的欧几里得距离平方
        inline float distance_squared(const Eigen::Vector3f& p1, const Eigen::Vector3f& p2)
        {
            return (p1 - p2).squaredNorm();
        }

        // FarthestPointSampling implementation - 最远点采样算法
        std::vector<int> FarthestPointSampling::sample(const Eigen::MatrixXf& points,
                                                       int num_samples)
        {
            int num_points = points.cols();
            if (num_samples >= num_points)
            {
                // 如果采样数量大于等于点数量，返回所有点的索引
        ...
        // BallQuery implementation - 球查询算法
        std::vector<std::vector<int>> BallQuery::query(const Eigen::MatrixXf& query_points,
                                                       const Eigen::MatrixXf& support_points,
                                                       float radius, int max_samples)
        {
            ...
            for (int i = 0; i < num_query_points; ++i)
            {
                ...
                // 如果邻居数量不足，重复最后一个邻居
                while (neighbors.size() < max_samples)
                {
        ...
        Eigen::MatrixXf MLPLayer::applyConv1d(const Eigen::MatrixXf& input, const ConvWeights& conv)
            const
        {
            // 1x1卷积：output = weight * input + bias
                                                 ...
                                                 const NormWeights& norm) const
        {
            ...
            for (int i = 0; i < output.rows(); ++i)
            {
                ...
                {
                    float normalized = (output(i, j) - norm.running_mean(i)) /
                                       std::sqrt(norm.running_var(i) + norm.eps);
        ...
        // SetAbstractionLayer implementation
        SetAbstractionLayer::SetAbstractionLayer(
            int npoint, const std::vector<float>& radii, const std::vector<int>& nsamples,
            const std::vector<MLPWeights>& mlp_weights, bool use_xyz)
        ...
        Eigen::MatrixXf SetAbstractionLayer::groupAndPool(
            const Eigen::MatrixXf& points, const Eigen::MatrixXf& new_points,
            const Eigen::MatrixXf& features, float radius, int nsample, const MLPLayer& mlp)
        {
            // 球查询
        ...
        // FeaturePropagationLayer implementation
        FeaturePropagationLayer::FeaturePropagationLayer(const MLPWeights& mlp_weights)
            : mlp_(mlp_weights)
        {
        }

        Eigen::MatrixXf FeaturePropagationLayer::forward(
            const Eigen::MatrixXf& points1, const Eigen::MatrixXf& points2,
            const Eigen::MatrixXf& features1, const Eigen::MatrixXf& features2)
        {
            if (points2.cols() == 0)
            {
                return features1;
            ...
            // 使用距离加权插值进行特征传播
            Eigen::MatrixXf interpolated_features =
                Eigen::MatrixXf::Zero(features2.rows(), points1.cols());
            ...
            for (int i = 0; i < points1.cols(); ++i)
            {
                ...
                std::partial_sort(distances.begin(),
                                  distances.begin() + std::min(k, (int)points2.cols()),
                                  distances.end());
