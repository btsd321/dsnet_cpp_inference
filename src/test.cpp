...
PointCloud createTestPointCloud(int num_points = 2500) {
    ...
    for (int i = 0; i < num_points / 3; ++i) {
        ...
        Point3D point(
            r * std::sin(theta) * std::cos(phi) + 1.0f,
...
bool testBasicFunctionality() {
    ...
    std::cout << "归一化前边界框: (" << bbox_before.first.x << "," << bbox_before.first.y << "," << bbox_before.first.z 
              << ") - (" << bbox_before.second.x << "," << bbox_before.second.y << "," << bbox_before.second.z << ")" << std::endl;
    std::cout << "归一化后边界框: (" << bbox_after.first.x << "," << bbox_after.first.y << "," << bbox_after.first.z 
              << ") - (" << bbox_after.second.x << "," << bbox_after.second.y << "," << bbox_after.second.z << ")" << std::endl;
...
bool testBatchInference() {
    ...
    try {
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            std::cout << "  - 场景" << (i + 1) << ": " << results[i].inference_time_ms 
                      << " ms, " << results[i].best_points.size() << " 个最佳点" << std::endl;
        }
...
bool testPerformance() {
    ...
    for (int size : test_sizes) {
        ...
        try {
            ...
            std::cout << "  点云大小: " << size << " -> 推理时间: " << duration.count() << " ms" << std::endl;
