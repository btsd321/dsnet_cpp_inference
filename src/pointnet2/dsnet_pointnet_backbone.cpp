...
std::vector<std::vector<int>> BallQuery::query(const Eigen::MatrixXf& query_points,
                                               const Eigen::MatrixXf& support_points, float radius,
                                               int max_samples)
{
...
Eigen::MatrixXf MLPLayer::applyConv1d(const Eigen::MatrixXf& input, const ConvWeights& conv) const
{
                                         ...
                                         const NormWeights& norm) const
{
    ...
    for (int i = 0; i < output.rows(); ++i)
    {
        ...
        {
            float normalized =
                (output(i, j) - norm.running_mean(i)) / std::sqrt(norm.running_var(i) + norm.eps);
...
FeaturePropagationLayer::FeaturePropagationLayer(const MLPWeights& mlp_weights) : mlp_(mlp_weights)
{
}
                                                 ...
                                                 const Eigen::MatrixXf& features2)
{
    ...
    // 使用距离加权插值进行特征传播
    Eigen::MatrixXf interpolated_features = Eigen::MatrixXf::Zero(features2.rows(), points1.cols());
    for (int i = 0; i < points1.cols(); ++i)
    {
        ...
        std::partial_sort(distances.begin(), distances.begin() + std::min(k, (int)points2.cols()),
                          distances.end());
        // 计算加权插值
